...
(function($) {
    ...
    $.imgAreaSelect = function (img, options) {
        ...
        function adjust() {
            ...
            if($img.hasClass('ninetydeg') || $img.hasClass('twoseventydeg'))	{
                if(!($.browser.msie && $.browser.version < 9)){
                    imgWidth = $img.innerHeight();
                    imgHeight = $img.innerWidth();

                    imgOfs = { left: round($img.offset().left  + 0.5*(imgWidth - imgHeight)), top: round($img.offset().top - 0.5*(imgWidth - imgHeight)) };
                } else {
                    imgWidth = $img.innerWidth();
                    imgHeight = $img.innerHeight();
